...
# CONFIG HELPER
#
# @description: Funktionen zum sicheren Laden & Validieren der serverbaukasten.conf
...
# Log-Funktionen (falls nicht schon global definiert)
...
# *_FILE-Resolver (no-op, wenn kein *_FILE gesetzt) + Secret-Erkennung
resolve_secret() {
...
# Heuristik zur Secret-Erkennung für Log-Maskierung
...
##
# Normalisiert eine Konfigurationsdatei (CRLF->LF, DOS-EOF entfernen, UTF-8 BOM)
...
normalize_config() {
    ...
    # Validierung
    [ -f "$src" ] || { log_error "Konfigurationsdatei nicht gefunden: $src"; exit 1; }
    
    # Temporäre Datei erstellen
    tmp="$(mktemp)" || { log_error "Konnte temporäre Datei nicht erstellen"; exit 1; }
    
    log_info "⚙️  Normalisiere Konfigurationsdatei..."
    
    # Datei kopieren
    cp "$src" "$tmp" || { log_error "Fehler beim Kopieren der Config"; rm -f "$tmp"; exit 1; }
    
    # Windows-Zeilenumbrüche prüfen und bereinigen
    if file "$tmp" 2>/dev/null | grep -q "CRLF\|with CR" || \
       od -c "$tmp" 2>/dev/null | head -1 | grep -q '\\r'; then
        log_warn "⚠️  Windows-Zeilenumbrüche (CRLF) erkannt - bereinige..."
        # CRLF zu LF konvertieren
        sed -i 's/\r$//' "$tmp"
        # DOS-EOF-Zeichen (^Z) entfernen
        sed -i 's/\x1a$//' "$tmp"
        log_ok "✅ Zeilenumbrüche bereinigt"
    fi
...
##
# Lädt Konfigurationsdatei sicher (ohne Code-Execution-Risiken)
...
source_config_safely() {
    ...
    # Erlaubt Quotes für Werte mit Leerzeichen, sperrt aber Substitution/Control-Operatoren
...
##
# Prüft, ob eine bedingte Validierung erfüllt ist
# @param string $1 Bedingung (leer, oder KEY=VALUE / KEY!=VALUE)
# @return int 0=Bedingung erfüllt, 1=nicht erfüllt
##
cond_met() {
    case "$1" in
        "") return 0 ;;  # Keine Bedingung = immer erfüllt
        *"!="*)
            local key="${1%%!=*}" val="${1#*!=}"
            [ "${!key:-}" != "$val" ]
            ;;
...
validate_config() {
    ...
    local validations=(
        ...
        # Mail-Konfiguration (nur wenn aktiviert)
        ...
        # Docker-Konfiguration (nur bei SERVER_ROLE=1)
        ...
        # GeoIP-Blocking (nur wenn aktiviert)
    ...
    for rule in "${validations[@]}"; do
        ...
        if cond_met "${cond#WHEN }"; then
            ...
            # Prüfe den Wert mit der Validierungsfunktion
            # Der Doppelpunkt ':' ist ein Platzhalter für reine Existenzprüfung
            if [ "$validator" != ":" ] && ! "$validator" "$val"; then
                local shown="$val"; is_secret_var "$var" && shown="***redacted***"
    ...
    # Optionale Variablen mit Defaults setzen (falls nicht gesetzt)
...
##
# Lädt und validiert die Konfiguration aus einer Datei (Hauptfunktion)
# @param string $1 Pfad zur Konfigurationsdatei
##
load_config_from_file() {
    ...
    # Optional: Secrets aus Dateien (macht nichts, wenn *_FILE fehlt)
