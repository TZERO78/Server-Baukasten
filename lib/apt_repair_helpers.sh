...
#
# @description: Provider-spezifische APT-Probleme erkennen & beheben
...
# Fallback-Logs, falls nicht global eingebunden -------------------------------
...
_have_proxy() {
  [ -n "${HTTP_PROXY:-}${http_proxy:-}${HTTPS_PROXY:-}${https_proxy:-}${NO_PROXY:-}${no_proxy:-}" ]
}
...
fix_apt_sources_if_needed() {
  ...
  if [ "$needs_fix" = true ]; then
    # Backups (sources.list + gesamte sources.list.d)
    local TS tmp; TS="$(date +%Y%m%d_%H%M%S)"
    [ -f /etc/apt/sources.list ] && cp -a /etc/apt/sources.list "/etc/apt/sources.list.backup.$TS"
    [ -d /etc/apt/sources.list.d ] && tar -C /etc/apt -czf "/etc/apt/sources.list.d.backup.$TS.tgz" sources.list.d 2>/dev/null || true

    # Sources atomisch generieren
    tmp=$(mktemp)
    case "$os_id" in
