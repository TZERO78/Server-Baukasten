...
## Preflight-Checks (kann später in 00_prepare_install.sh wandern)
pre_flight_checks() {
    ...
    declare -A cmd_to_pkg=(
        [curl]="curl" [wget]="wget" [gpg]="gnupg" [systemctl]="systemd"
        [ip]="iproute2" [apt-get]="apt" [sed]="sed" [envsubst]="gettext-base" [logger]="bsdutils"
    )
    ...
    if [ ${#missing_cmds[@]} -gt 0 ]; then
        ...
        log_info "  -> Versuche automatische Installation: ${missing_pkgs[*]}"

        if ! type -t fix_apt_sources_if_needed &>/dev/null; then
            log_debug "  -> Lade lib/apt_repair_helpers.sh..."
            [ -f "lib/apt_repair_helpers.sh" ] || { log_error "❌ APT-Reparatur-Helper fehlt."; exit 1; }
        ...
        local update_attempts=0 max_attempts=3 update_success=false
        while [ $update_attempts -lt $max_attempts ] && [ "$update_success" = false ]; do
            ((update_attempts++))
            log_debug "  -> APT-Update Versuch $update_attempts/$max_attempts"
            local out; out=$(apt-get update 2>&1); local rc=$?
            if [ $rc -eq 0 ]; then
                update_success=true
                local repo_count; repo_count=$(echo "$out" | grep -c -E '^(Get:|Hit:)')
                log_debug "  -> $repo_count Repositories verarbeitet"
            else
                log_warn "  -> APT-Update fehlgeschlagen (Exit: $rc)"
                echo "$out" | grep -q "Could not get lock" && { log_debug "  -> APT gesperrt, warte..."; sleep 10; }
                echo "$out" | grep -q "NO_PUBKEY" && { log_debug "  -> GPG-Schlüssel fehlen (apt-key update)"; apt-key update 2>/dev/null || true; }
                [ $update_attempts -lt $max_attempts ] && { log_debug "  -> retry in 5s..."; sleep 5; } || {
        ...
        local install_out; install_out=$(DEBIAN_FRONTEND=noninteractive apt-get install -y "${missing_pkgs[@]}" 2>&1); local irc=$?
        if [ $irc -eq 0 ]; then
            log_ok "✅ Fehlende Pakete installiert: ${missing_pkgs[*]}"
            for pkg in "${missing_pkgs[@]}"; do
                dpkg -l | grep -q "^ii.*$pkg" && log_debug "  -> ✓ $pkg installiert" || log_debug "  -> ⚠ $pkg Status unklar"
            done
        else
            log_error "❌ Installation fehlgeschlagen (Exit: $irc)"
            echo "$install_out" | grep -E "^E:|^W:" | head -10
            echo "$install_out" | grep -q "dpkg was interrupted" && {
                log_info "  -> Repariere dpkg (--configure -a)"; dpkg --configure -a || true
                log_info "  -> Erneuter Installationsversuch..."
                apt-get install -y "${missing_pkgs[@]}" || exit 1
            } || exit 1
        fi
        ...
        for cmd in "${missing_cmds[@]}"; do
            command -v "$cmd" &>/dev/null || { still_missing+=("$cmd"); log_debug "  -> ❌ $cmd fehlt weiterhin"; }
        done
        [ ${#still_missing[@]} -eq 0 ] || { log_error "❌ Befehle fehlen weiterhin: ${still_missing[*]}"; log_debug "  -> PATH: $PATH"; exit 1; }
        ...
        if ! type -t detect_vps_provider &>/dev/null; then
            ...
            [ -f "lib/apt_repair_helpers.sh" ] && source lib/apt_repair_helpers.sh
        fi
        if type -t detect_vps_provider &>/dev/null; then
            local vps_provider; vps_provider=$(detect_vps_provider); export VPS_PROVIDER="$vps_provider"
            log_debug "  -> VPS-Provider: ${VPS_PROVIDER}"
        fi
        log_debug "  -> Alle ${#cmd_to_pkg[@]} kritischen Befehle vorhanden"
    fi

    # Sicherer Check (mit Default) um set -u zu vermeiden
    if [ "${VPS_PROVIDER:-}" = "ionos" ] || [ -d /etc/apt/mirrors ]; then
        log_warn "  -> IONOS/Problem-Provider erkannt - prüfe APT-Quellen..."
        type -t fix_apt_sources_if_needed &>/dev/null && fix_apt_sources_if_needed || true
    fi
